...
// TODO: MODIFIED BY ZHAO assign sample_indices + update cloud_processed_
// when sample_indices changed, the function should be called!
// by sample_indices_
void Cloud::update()
{
    if(sample_indices_.size()>0){
        Eigen::MatrixXi camera_source(camera_source_.rows(), sample_indices_.size());
        PointCloudRGB::Ptr cloud(new PointCloudRGB);
        cloud->points.resize(sample_indices_.size());
        for (int i = 0; i < sample_indices_.size(); i++) {
            camera_source.col(i) = camera_source_.col(sample_indices_[i]);
            cloud->points[i] = cloud_processed_->points[sample_indices_[i]];
        }
        if (normals_.cols() > 0) {
            Eigen::Matrix3Xd normals(3, sample_indices_.size());
            for (int i = 0; i < sample_indices_.size(); i++) {
                normals.col(i) = normals_.col(sample_indices_[i]);
            }
            normals_ = normals;
        }
        cloud_processed_ = cloud;
...
void Cloud::filterSamples(const std::vector<double> &workspace) {
  ...
  for (int i = 0; i < indices.size(); i++) {
      // TODO: MODIFIED BY ZHAO
//    filtered_samples.col(i) = samples_.col(i);
    filtered_samples.col(i) = samples_.col(indices[i]);
  }
...
void Cloud::subsampleUniformly(int num_samples) {
  ...
  // TODO: MODIFIED BY ZHAO change cloud_processed_
  // random_sample.filter(*cloud_processed_);
    update();
}
...
//  std::vector<int> indices(num_samples);
  ...
  for (int i = 0; i < num_samples; i++) {
      set_indices.insert(sample_indices_[rand() % sample_indices_.size()]);
//    indices[i] = sample_indices_[rand() % sample_indices_.size()];
  }
  std::vector<int> indices(set_indices.size());
  size_t i=0;
  for(int set_indice : set_indices)
  {
      indices[i]=set_indice;
      ++i;
  }
...
// TODO: MODIFIED BY ZHAO only function to set samples_
